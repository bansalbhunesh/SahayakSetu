...
Auto-detects Redis reachability at startup. If the configured Redis URL is
unreachable (DNS failure, network partition, Upstash region outage), the limiter
silently falls back to in-memory storage so the API keeps serving requests.
"""
...
def _safe_storage_uri() -> str:
    ...
    if raw.startswith(("redis://", "rediss://")):
        ...
        # Unknown scheme in override — be safe, use in-memory.
        return "memory://"
    else:
        use_redis = _env_bool(
    ...
    if is_reachable():
        return candidate
    print(
        f"[WARN] Redis at {candidate[:40]}… is unreachable; falling back to in-memory rate limiting.",
        flush=True,
    )
    return "memory://"
